      ...
      if (is_birth==0)  {
        ...
        GR_foetus_sim[i_day:max_days[i_sim],i_sim] <- S_foetus_sim[i_day:max_days[i_sim],i_sim] <- 0} else {
          ...
          if (calf_days > 0) {
            ...
            # modification to include the fact that harbour seal feed during lactation
            R_sim[(calf_birth_day-4):(calf_birth_day+Lact_feed)] <- 0
            R_sim[(calf_birth_day+Lact_feed):end_lact] <- R_prop_lactation*R_sim[end_lact+1]
